...
# 🔁 선 연결: 각 측점 기준으로 가장 가까운 5개의 거리만 계산
distance_data = []

for region, group in df_point.groupby("해역"):
    locations = list(zip(group['측점'], group['위도'], group['경도']))
    for i, (p1, lat1, lon1) in enumerate(locations):
        distances = []
        for j, (p2, lat2, lon2) in enumerate(locations):
            if i == j:
                continue
            km = round(geodesic((lat1, lon1), (lat2, lon2)).km, 2)
            distances.append((km, p1, p2, lat1, lon1, lat2, lon2))
        distances.sort()  # 가까운 거리 순 정렬
        for entry in distances[:4]:  # 최대 5개만 선택
            km, p1, p2, lat1, lon1, lat2, lon2 = entry
